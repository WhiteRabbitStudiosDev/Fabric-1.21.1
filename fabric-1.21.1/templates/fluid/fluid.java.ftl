...
public abstract class ${name}Fluid extends FlowingFluid {
	...
	@Environment(EnvType.CLIENT) public static void clientLoad() {
		final ResourceLocation stillTexture = ResourceLocation.parse("${data.textureStill.format("%s:block/%s")}");
		final ResourceLocation flowingTexture = ResourceLocation.parse("${data.textureFlowing.format("%s:block/%s")}");
		final ResourceLocation overlayTexture = <#if data.textureRenderOverlay?has_content>ResourceLocation.parse("${data.textureRenderOverlay.format("%s:textures/%s")}.png")<#else>null</#if>;

		FluidRenderHandlerRegistry.INSTANCE.register(
			${JavaModName}Fluids.${REGISTRYNAME},
			${JavaModName}Fluids.FLOWING_${REGISTRYNAME},
			new SimpleFluidRenderHandler(stillTexture, flowingTexture, overlayTexture) {
				<#if data.isFluidTinted()>
				public int getFluidColor(@Nullable BlockAndTintGetter world, @Nullable BlockPos pos, FluidState state) {
