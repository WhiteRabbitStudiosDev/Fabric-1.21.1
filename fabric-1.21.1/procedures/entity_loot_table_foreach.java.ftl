...
if (${input$entity} instanceof LivingEntity _entLootTbl && _entLootTbl.level().getServer() != null) {
	...
	for (ItemStack itemstackiterator : _entLootTbl.level().getServer().reloadableRegistries().getLootTable(ResourceKey.create(Registries.LOOT_TABLE, ${toResourceLocation(input$location)}))
